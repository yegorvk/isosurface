...
use crate::distance::{Directed, Signed};
use glam::Vec3;
...
pub struct CentralDifference<'a, S: ScalarSource + ?Sized> {
    pub source: &'a S,
    epsilon: f32,
}

impl<'a, S: ScalarSource + ?Sized> CentralDifference<'a, S> {
    ...
    /// value.
    pub fn new_with_epsilon(source: &'a S, epsilon: f32) -> Self {
        Self { source, epsilon }
    }
}

impl<S: ScalarSource + ?Sized> ScalarSource for CentralDifference<'_, S> {
    fn sample_scalar(&self, p: Vec3) -> Signed {
        self.source.sample_scalar(p)
    }
}

impl<S: VectorSource + ScalarSource + ?Sized> VectorSource for CentralDifference<'_, S> {
    fn sample_vector(&self, p: Vec3) -> Directed {
        self.source.sample_vector(p)
    }
}

impl<S: ScalarSource + ?Sized> HermiteSource for CentralDifference<'_, S> {
    fn sample_normal(&self, p: Vec3) -> Vec3 {
