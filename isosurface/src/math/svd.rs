//! This is a mostly machine translation of the SVD from Ronen Tzur's dual contouring sample
...
impl SVD {
    pub fn new(mat: &[[f64; 3]]) -> Self {
        ...
        // matrix 3 x 3 (for 3 columns in mat).
...
unsafe extern "C" fn factorize(
    ...
    let mut y: usize = 0;
    // bi-diagonal factorization of (rows x 3) matrix into :-
...
unsafe extern "C" fn factorize_hh(mut ptrs: *mut *mut f64, mut n: usize) -> f64 {
    ...
    if n > 1 as usize {
        let mut x_norm: f64 = 0.;
        if n == 2 as usize {
            x_norm = (**ptrs.offset(1 as usize as isize)).abs()
        } else {
        ...
        if x_norm != 0.0f64 {
            ...
            let mut beta: f64 = (alpha * alpha + x_norm * x_norm).sqrt();
            if alpha >= 0.0f64 {
                beta = -beta
            }
            tau = (beta - alpha) / beta;
