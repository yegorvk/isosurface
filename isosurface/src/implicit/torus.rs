...
impl VectorSource for Torus {
    fn sample_vector(&self, p: Vec3) -> Directed {
        ...
        //     a.try_normalize().unwrap_or(Vec3::new(0.0, 0.0, 1.0)) * self.tube_radius;
...
mod tests {
    ...
    fn test_torus() {
        ...
        assert_eq!(
            ...
            Vec3::new(6.0, 6.0, f32::MAX)
        );
        assert_eq!(
            torus.sample_vector(Vec3::new(8.0, 0.0, 0.0)).0,
            Vec3::splat(-2.0),
        );
        assert_eq!(torus.sample_vector(Vec3::new(10.0, 0.0, 0.0)).0, Vec3::ZERO);
        assert_eq!(
            torus.sample_vector(Vec3::new(12.0, 0.0, 0.0)).0,
            Vec3::new(2.0, f32::MAX, f32::MAX)
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Vec2::splat(9.0 - (10.0 * 10.0 - 9.0 * 9.0f32).sqrt()).extend(f32::MAX)
        );
        assert_eq!(
            torus.sample_vector(Vec3::new(2.0, 2.0, 0.0)).0,
            Vec2::splat((6.0 * 6.0 - 2.0 * 2.0f32).sqrt() - 2.0).extend(f32::MAX)
        );
        assert_eq!(
