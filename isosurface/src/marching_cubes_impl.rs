...
{
    for i in 0..8 {
        normals[i] = source.sample_normal(corners[i]).normalize_or_zero()
    }
}
    ...
    S: Sample<D> + HermiteSource,
{
    ...
    for i in 0..12 {
        if (edges & (1 << i)) != 0 {
            normals[i] = source.sample_normal(vertices[i]).normalize_or_zero()
        }
    }
}
    ...
    F: FnMut(usize, usize, usize),
{
    for i in 0..5 {
        ...
        face_callback(
            TRIANGLE_CONNECTION[cube_index][3 * i] as usize,
